...
**Given**: 新規ドキュメントで PDF 変換設定なし
**When**: ゼロから PDF 変換を実行
**Then**: 最適な変換パイプラインを選択し PDF を出力する

### 不正なファイルでのエラーハンドリング

**Given**: 破損した PDF ファイルやパスワード保護された入力
**When**: PDF 変換・抽出を実行
**Then**: エラー種別と対処法を警告メッセージで出力する

### diagram スキルとの連携

**Given**: diagram スキルで生成済みの図表ファイル
**When**: 上流スキルの成果物を参照して PDF に統合
**Then**: 図表を含む統合 PDF ドキュメントを出力する
...
| `project/00_schemas/DOCUMENT_STYLE_TEMPLATE.md` | PDF 変換スタイルテンプレート |
| `project/00_schemas/REVIEW_RESULT_TEMPLATE.md` | 変換品質レビュー結果テンプレート |
...
- [ ] 変換後の見出し階層が元文書の目次・構造を忠実に再現しているか
- [ ] テーブル構造が元文書と一致しているか
- [ ] 図表のキャプション・凡例・説明文が保持されているか
- [ ] ヘッダー/フッター/ページ番号が適切に除去されているか
- [ ] OCR 言語設定が文書の言語に合わせて設定されているか
- [ ] 変換後の Markdown が distill スキルの入力として使用可能な品質か
...
| `.agents/workflows/convert-pdf.md` | エージェントワークフロー定義 |
