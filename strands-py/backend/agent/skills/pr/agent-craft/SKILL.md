...
- [ ] エージェントの Role 記述が汎用的すぎないか（「あなたは優秀なアシスタントです」等のテンプレ表現）
- [ ] maxTurns・終了条件が明示されているか（無限ループリスクの排除）
- [ ] ツール制限（allowedTools）がエージェントの責務に対して適切に設定されているか
- [ ] 複数エージェント構成で各エージェントのスタンスが差別化されているか（同じ結論に収束しない設計）
- [ ] プロンプトが 500 語を超えていないか（超過時は参照ファイルに分離すべき）
- [ ] 出力形式（Output Format）が具体的に指定されているか（入出力例を含む）
