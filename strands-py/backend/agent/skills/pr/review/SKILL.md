...
- 個人の好みに基づくコーディングスタイル指摘をレビューコメントに含めてはならない
- 修正提案なしの否定的コメントのみを残さないこと
...
### 3g: State Alignment（ペルソナ心理状態の整合性）

ペルソナの State Dimensions（Belief/Value/Stance/Emotion/Communication）が、
ストーリーの受入基準・テストデータ・UX 設計に一貫して反映されているか検証する。
...
> **適用条件**: State Dimensions が `20_notes/` または story-map 成果物に定義されている場合に適用。
> 未定義の場合は Info レベルで「State Dimensions の定義を推奨」と指摘する。
...
- [ ] レビュー指摘が Critical/Warning/Suggestion の重要度で分類されているか
- [ ] 問題点だけでなく具体的な修正案がセットで提示されているか
- [ ] レビュー対象のドメイン固有の基準（コーディング規約・設計原則等）に基づいているか（汎用的な「改善の余地あり」を排除）
- [ ] 良い点の指摘が含まれているか（問題点のみの一方的なレビューになっていないか）
- [ ] レビュー結果の JSON 出力が REV-YYYY-MM-DD-NNN 形式で保存可能な構造か
- [ ] 数値データの整合性チェックが含まれているか（data-validation との連携を確認）
