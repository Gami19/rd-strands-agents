...
| ビジネスルール | `.md` | `notes/business-rules.md` |
| 非機能要件 | `.md` | `notes/non-functional-requirements.md` |
| API 仕様（該当時） | `.md` | `notes/api-spec.md` |
...
設計判断が発生した時点で `decision_log/` に ADR 形式で記録する。

```
# ADR ファイル命名規則
decision_log/ADR-NNNN-<判断概要>.md

# 例
decision_log/ADR-0001-アーキテクチャ選定-Data-Lakehouse.md
decision_log/ADR-0002-コンピュート選定-Serverless.md
decision_log/ADR-0003-認証方式-Entra-ID.md
decision_log/ADR-0004-パーティション戦略-Liquid-Clustering.md
```
...
品質ゲート通過（Phase 3）
  ...
  │
  └─▶ リリース判断の記録
        └─ decision_log/ADR-NNNN-リリース判断-*.md
```
...
```
<project-root>/
├── inbox/              ← Phase 0: 仕様書・設計書（Markdown 化済み）
├── notes/              ← Phase 0: 蒸留済み要件（用語集、ドメインモデル、ビジネスルール）
│   ├── glossary.md
│   ├── domain-model.md
│   ├── business-rules.md
...
├── decision_log/       ← Phase 0-4: ADR（設計判断記録）
│   ├── ADR-0001-*.md
│   └── ...
├── references/         ← 外部参照資料
├── .agent/
...
```
1. 「API 仕様を inbox に取り込んで蒸留して」               → pdf-convert + distill
