...
| スキル | 役割 | トリガー例 |
|:---|:---|:---|
| **pdf-convert** | 仕様書・RFP・技術資料の Markdown 変換 | 「仕様書を inbox に取り込んで」 |
| **distill** | 要件の蒸留・構造化（用語集、ドメインモデル、非機能要件） | 「inbox を notes に蒸留して」「用語集を作って」 |
...
| 成果物 | 形式 | 配置先 |
|:---|:---|:---|
| 仕様書（Markdown 化済み） | `.md` | `inbox/` |
| 用語集 | `.md` | `notes/glossary.md` |
| ドメインモデル | `.md` | `notes/domain-model.md` |
| 非機能要件 | `.md` | `notes/non-functional-requirements.md` |
| ストーリーマップ | `.md` | `notes/story-map.md` |

### チェックリスト（Phase 0 → Phase 1 ゲート）

- [ ] 全仕様書が `inbox/` に Markdown 化されている
- [ ] 用語集でドメイン用語が統一されている
...
| ADR（設計判断記録） | `.md` | `decision_log/` |
| DACI / RACI チャート | `.md` | `docs/architecture/` |

### ADR 記録

設計判断が発生した時点で `decision_log/` に ADR 形式で記録する。decision-framework の 7 セクション構造を使用。

```
# ADR naming convention
decision_log/ADR-NNNN-<summary>.md

# Examples
decision_log/ADR-0001-architecture-style-service-based.md
decision_log/ADR-0002-data-architecture-lakehouse.md
decision_log/ADR-0003-communication-protocol-grpc.md
decision_log/ADR-0004-tenant-isolation-schema-per-tenant.md
```
...
| DRM 台本（アーキテクチャ承認） | `.md` | `snapshots/` |
| 最終 ADR（移行戦略、承認判断） | `.md` | `decision_log/` |
...
| Phase 2 で構成図が不完全 | コンポーネント間のインターフェース未定義 | notes の API 仕様を先に策定。BC の Context Map パターンを参照 |
...
```
<project-root>/
├── inbox/                 ← Phase 0: 仕様書・技術資料（Markdown 化済み）
├── notes/                 ← Phase 0: 蒸留済み要件
│   ├── glossary.md
│   ├── domain-model.md
...
│   └── reviews/              レビューレポート
├── decision_log/          ← Phase 2-4: ADR（設計判断記録）
│   ├── ADR-0001-*.md
│   └── ...
├── snapshots/             ← Phase 4: DRM 台本
├── references/            ← 外部参照資料
├── .agent/
│   ├── skills/               導入済みスキル群
│   └── workflows/            ワークフロー定義
...
```
1. 「この仕様書を inbox に取り込んで」                             → pdf-convert
2. 「inbox を notes に蒸留して」                                   → distill
...
```
1. 「現行システムの情報を inbox に取り込んで蒸留して」             → pdf-convert + distill
